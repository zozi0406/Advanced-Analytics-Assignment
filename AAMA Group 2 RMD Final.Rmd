...
------------ | -----------------------------------------------------------------------------
`STORE`      | Store Number
`UPC`        | Last 5 digits of the UPC number identifies a product, the remaining digits identify the manufacturer
`WEEK`       | Week Number
`MOVE`       | Number of units sold. MOVE represents the number of individual units sold, not bundles
`QTY`        | Size of the bundle (e.g. 3)
`PRICE`      | Total price of bundle
`SALE`       | Indicates whether the product was sold on promotion that week. “B” indicates a bonus buy, “C” indicates a coupon, “S” indicates a small price reduction
`PROFIT`     | Gross margin in percent that store makes on the sale. A profit of 25.3 means that the store makes 25.3 cents for every dollar for each item sold
`OK`         | Flag set by the authors to indicate that the data for that week was suspect
`PRICE_HEX`  | Full precision in hexadecimal notation of `PRICE`.
`PROFIT_HEX` | Full precision in hexadecimal notation of `PROFIT`
